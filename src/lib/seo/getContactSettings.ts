...
export async function getContactSettings(): Promise<ContactSettings> {
  ...
  // E-posta domain bağımsız: site.json'dan domain al ve info@domain oluştur
  const siteSettings = await getSiteSettings();
  if (siteSettings.domain && (!contactData.email || contactData.email === 'info@example.com')) {
    contactData.email = `info@${siteSettings.domain}`;
  }
